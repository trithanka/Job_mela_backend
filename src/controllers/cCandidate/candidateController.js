...
const getMatchingJobs = async (candidateId, qualification) => {
    ...
    try {
        ...
        throw new Error('Internal Server Error during job fetching: ' + error.message);
    } finally {
...
const asdmTrained=async(req,res) =>{
    ...
    try {
        try {
            const contactCheck = await connection.query(mysqlDB, Query.checkCandidateasdm, [contactNumber,dob]);
                
            if (contactCheck.length == 0) {
                return res.status(400).json({ status: false, message: "Phone number is not Registered" });
            }
        } catch (error) {
        ...
        try {
            let insertData;
            const mydata=await connection.query(mysqlDB, Query.checkCandidateAsdm, [contactNumber]);
            if(req.body.insert){
                try {
                    //generate date,id
                    ...
                    //insert data
                    insertData=await connection.query(mysqlDB, Query.addCandidateAsdm, [candidateId, mydata[0].fullName, mydata[0].contactNumber, mydata[0].emailAddress, mydata[0].fklQualificationId, mydata[0].dob, mydata[0].area, mydata[0].city, mydata[0].state, mydata[0].pinCode, registrationDate,1,fklmela_no,mydata[0].pklCandidateId]);
...
const portalCan=async(req,res) =>{
    ...
    try {
        try {
            const contactCheck = await connection.query(mysqlDB, Query.checkCandidatePor, [contactNumber,dob]);
                
            if (contactCheck.length == 0) {
                return res.status(400).json({ status: false, message: "Phone number Or DOB Is Not Matched" });
            }
        } catch (error) {
            return res.status(500).json({ status: false, message: "Internal Server Error contact", error: error.message });
        }
        try {
            // let insertData;
            const mydata=await connection.query(mysqlDB, Query.checkCandidatePortal, [contactNumber]);
            // if(req.body.insert){
            //     try {
            //         //generate date,id
            //         const registrationDate = new Date();
...
const updateCandidate = async (req, res) => {
    ...
    const { contactNumber } = req.params;
    let { fullName, emailAddress, qualification, dob, area, city, state, pinCode, fklmela_no,fklCandidateId} = req.body;
    if (emailAddress === "") {
        emailAddress = null;
    }
    ...
    try {
        ...
        await connection.query(mysqlDB, Query.updateCandidatejp, [
            fullName, emailAddress, formattedDob,fklCandidateId
        ]);
        //update address
        await connection.query(mysqlDB, Query.updateCandidatejpAdd, [
            area, city, state,fklCandidateId
        ]);
        //update education
        await connection.query(mysqlDB, Query.updateCandidatejpEdu, [
            qualification,fklCandidateId
        ]);
        /////////////////////////////////////////////////////////////////////////////////////////////////

        //update in job mela table
        await connection.query(mysqlDB, Query.updateCandidate, [
            fullName, emailAddress, qualification, formattedDob, area, city, state, pinCode,fklmela_no ,contactNumber
        ]);
        
        const updatedCandidateData = await connection.query(mysqlDB, Query.getCandidate, [contactNumber]);
        ...
        const showJob=await getMatchingJobs(updatedCandidateData[0].candidate_id,qualification)

        res.status(200).json({ status: true, message: "Candidate updated successfully", data: updatedCandidateData, jobs:showJob });
    } catch (error) {
        res.status(500).json({ status: false, message: "Internal Server Error", error: error.message });
    }finally {
        if (mysqlDB) mysqlDB.release();
...
const getAllCandidate = async (req, res) => {
    ...
    try {
        ...
        if (req.body.type=="placed"){
            candidateData=await connection.query(mysqlDB,Query.selectedCandidateAll)
...
const master=async(req,res)=>{
    ...
    try {
         ...
         const totalCandidates = await connection.query(mysqlDB, Query.totalCandidate);
        ...
        const melaInfo = await connection.query(mysqlDB, Query.melaInfo);
         ...
         const appliedApplicant=await connection.query(mysqlDB,Query.appliedApplicant)
         const appliedCandidate=await connection.query(mysqlDB,Query.appliedCandidate)
         //Query for selected applicant and candidaate
         const selectedApplicant=await connection.query(mysqlDB,Query.selectedApplicant)
         const selectedCandidate=await connection.query(mysqlDB,Query.selectedCandidate)
         //Query for rejected applicant and candidaate
         const rejectedApplicant=await connection.query(mysqlDB,Query.rejectedApplicant)
         const rejectedCandidate=await connection.query(mysqlDB,Query.rejectedCandidate)
         //qualification
