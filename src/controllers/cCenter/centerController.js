...
const allCandidate = async (req, res) => {
    ...
    const { candidate_id, company_name, full_name, registration_no, mela_id } = req.body
    try {
...
const checkIn = async (req, res) => {
    ...
    try {
        ...
        let { melaId, candidateId, checkInDate } = req.body
        //check if date is in the format of yyyy-mm-dd
        if (!/^\d{4}-\d{2}-\d{2}$/.test(checkInDate)) {
            return res.status(400).json({ status: false, message: "date must be in the format of yyyy-mm-dd" });
        }

        if (!melaId || !candidateId || !checkInDate) {
            return res.status(400).json({ status: false, message: "all fields are required" });
        }

        //find out the job which are available in the specific date and check if the candidate is already checked in same query
        const availableJob = await connection.query(mysqlDB, uQuery.availableJob, [melaId, candidateId, checkInDate]);
        if (availableJob.length === 0) {
            return res.status(400).json({ status: false, message: "no job available or already checked in" });
        }
        // console.log(availableJob);



        availableJob.forEach(async (job) => {
            //get the last interview time of candidate for that job
            const result = await connection.query(mysqlDB, uQuery.lastInterviewTime, [job.jobId]);
            let newInterviewTime;

            if (result[0].lastInterviewTime !== null) {
                //add interviewDurationMin to the lastInterviewTime and make it does not exceed the interviewEndTime
                const lastInterviewTime = new Date(result[0].lastInterviewTime);
                newInterviewTime = new Date(lastInterviewTime.getTime() + job.interviewDurationMin * 60000);
            } else {
                //add interviewDurationMin to the checkInDate and make it does not exceed the interviewEndTime
                const lastTime = new Date(checkInDate);
                newInterviewTime = new Date(lastTime.getTime() + job.interviewDurationMin * 60000);
            }

            const interviewEndTime = new Date(job.interviewEndTime);
            //check if the newInterviewTime is greater than the interviewEndTime
            if (newInterviewTime > interviewEndTime) {
                return res.status(400).json({
                    status: false,
                    message: "Interview time exceeds the interview end time"
                });
            }

            console.log("New Interview Time:", newInterviewTime);
...
module.exports = { loginUser, allCandidate, jobDetail, jobApply, allCompany, allJob, checkIn }